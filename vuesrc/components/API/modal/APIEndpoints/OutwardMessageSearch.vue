      ...
      <div class="row">
        <div class="col-md-12" v-if="view == 'description'">
          <p class="description-content">
            Originating Financial Institutions that send an optional, Local
            External Code may also perform queries to search for a specific
            Outward Message without the original Outward Message Sequence
            Number.
          </p>

          <p class="description-content">
            This may be useful when an OFI is for any reason unable to use the
            original Outward Message Sequence Number, or for verifying if a
            failed (or timed out) invocation was successfully processed or not
            without having to reconcile and iterate messages through the Index
            Endpoint.
          </p>

          <form class="send-outward-message-search">
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label class="form-control-label">LOCAL EXTERNAL CODE</label>
                  <input
                    type="text"
                    class="form-control"
                    name="localExternalCode"
                    v-model="localExternalCode"
                  />
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      const valid = $(".send-outward-message-search").valid();
      ...
      const { status, data, responseStatus } = await this.$store.dispatch(
        ...
        {
          localExternalCode: this.localExternalCode,
        }
      );
  ...
  mounted() {
    $(".send-outward-message-search").validate({
      rules: {
        localExternalCode: {
          required: true,
        },
