<template>
  <div>
    <div class="modal-body">
      ...
      <form class="create-outward-message-form" v-if="!isLoading">
        ...
        <div class="row" v-if="outwardData.sequenceNumber">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input
                type="text"
                class="form-control"
                :value="outwardData.sequenceNumber"
                readonly="true"
              />
            </div>
          </div>
          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input
                type="text"
                class="form-control"
                :value="outwardData.settlementDate"
                readonly="true"
              />
            </div>
          </div>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <label class="form-control-label"
                        ...
                        <b class="text-danger" v-if="!transaction.sent"
                          >{{
                            hasDuplicateMessage(
                              transaction.ofiReferenceNumber,
                              index
    ...
    <div
      class="modal-footer"
      v-if="
        isLoading == 0 && outwardData.processing == 0 && outwardData.status != 2
      "
    >
      <button
        type="button"
      ...
      <button
        type="button"
        class="btn btn-success"
        :disabled="removeRack.length == 0 || errorCount > 0"
    ...
    isFull() {
      return (
        this.transactions
          ...
          .filter((data) => data != undefined).length == this.removeRack.length
      );
    },
  ...
  methods: {
    ...
    async getData() {
      ...
      const [
        ...
        await this.$store.dispatch("outward/getTransactionsRaw", {
          referenceId: this.outwardData.referenceId,
        }),
      ]);
        ...
        buttons: {
          submit: {
            ...
            action() {
              ...
              vm.$store
                .dispatch("outward/saveOutwardMessage", payload)
                .then((res) => {
                  ...
                  if (status == 200) {
                    vm.getData().then((res) => {
                      vm.transactions.filter((data) => data.sent).length ==
            ...
            action() {
              ...
              vm.$store
                ...
                .then((res) => {
                  ...
                  const isSent = message.includes(
                    "Transaction/s are already sent."
                  );

                  JCinstance.setContent(`<b>${message}</b>`);
      ...
      if(!vm.outwardData.processing && vm.outwardData.isSent && !vm.outwardData.status && vm.outwardData.remarks != 'validated'){
        this.$popover({
          ...
          onContentReady: function() {
            ...
            vm.$store
              .dispatch("outward/validateOutwardMessage", payload)
              .then((res) => {
                const { status, message, errors, data } = res;
                ...
                if (status == 200) {
                  ...
                  const errorContent = "Failed to validate batch.";
                  JCinstance.$$submit.show();
                  JCinstance.$$close.show();
                }
              });
