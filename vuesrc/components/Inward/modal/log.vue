<template>
  <div
    ...
    tabindex="-1"
    role="dialog"
    aria-labelledby="inward-log-modal"
    aria-hidden="true"
  >
    <div
...
export default {
  ...
  props: ["viewInwardLog", "logId"],

  watch: {
    async viewInwardLog(newVal, oldVal) {
      if (newVal == 1) {
        $("#inward-log-modal").modal({
          backdrop: "static",
        });
  ...
  methods: {
    closeModal() {
      $("#inward-log-modal").modal("hide");
      this.$emit("update:viewInwardLog", 0);
    },
